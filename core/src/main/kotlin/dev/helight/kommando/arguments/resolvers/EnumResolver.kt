...
class EnumResolver: ArgumentResolver {
    override val priority: Int = 0
    override val isContextOnly: Boolean = false

    override fun associatedAnnotation(): Collection<Class<*>> = listOf()
    override fun associatedTypes(): Collection<Class<*>> = listOf(Enum::class.java)
    ...
    override fun deserialize(
        ...
        println("Trying to decode enum")
        val query = args.pop()
        println("Values: ${argumentContext.type.enumConstants.joinToString(", ")}")
        return argumentContext.type.enumConstants.firstOrNull {
            val string = it.toString()
            val dotSpliced = string.split(".")
            if (dotSpliced.size == 1) {
                dotSpliced[0].equals(query, ignoreCase = true)
            } else {
                dotSpliced.getOrNull(1)?.equals(query, ignoreCase = true) ?: false
            }
        }
    }
}